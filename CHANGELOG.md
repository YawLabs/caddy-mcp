...
- **`caddy_config_by_id` no longer replaces or unloads the entire config, or reaches other
  ...
  Where `/id/<id>` goes is decided by Caddy's own index, and an id can resolve to the
  config root: a top-level `"@id"`, string or number (Caddy keys a number by its Go `%v`
  string), or a nested `@id` under keys whose `..` Caddy's `path.Join` collapses
  (`caddy.go:311` at v2.11.4). There, `PATCH` and `POST` replace the whole config, `PUT`
  answers 409 and `DELETE` unloads it, the `admin` block included. An id can also collapse
  *outside* the config tree, to `/` or `/load`, where `/id/<id>/load` is a whole-config
  load and `/id/<id>/stop` would stop Caddy. All verified against Caddy 2.11.4. `set` had
  no `confirm` gate, and neither action took a snapshot or said where the admin endpoint
  went. Every `set` and `delete` now asks Caddy where the id resolves first, via `GET
  /id/<id>/@id`, whose ETag carries the resolved path. An id that does not resolve inside
  the config tree is refused with nothing sent, and that includes an unknown id, with
  Caddy's own error passed through. An id that resolves to the root, with a subpath that
  stays at it (empty, slashes only, or a lone `...`), takes the branch a root
  `caddy_config_set` or `caddy_config_delete` does: refused without `confirm=true`, then
