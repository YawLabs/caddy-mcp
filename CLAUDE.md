...
- `src/snapshots.ts` — Ring of the last 10 config snapshots (module-level, per-process). Backs `caddy_revert`; auto-captured before `caddy_load`, and before a `caddy_config_delete`, `caddy_config_set` or `caddy_config_by_id` at the config root (no other delete or set is snapshotted). A `caddy_config_by_id` call is at the root when its `id` is the config's own top-level `@id` and its subpath is empty or a lone `...`; the tool reads the top-level `@id` to find out, and all three tools share one root write branch and one root delete branch in `src/tools/config.ts`. Whether a path IS the root is `api.isRootConfigPath`, which shares `normalizePath` with the request functions and also absorbs the lone trailing `...` segment Caddy strips before it picks a method. In-memory by default; `CADDY_MCP_SNAPSHOT_DIR` persists them to disk and rehydrates on first access, so rollback survives a restart.
