...
- `src/snapshots.ts` — Ring of the last 10 config snapshots (module-level, per-process). Backs `caddy_revert`; auto-captured before `caddy_load`, and before a `caddy_config_delete` or `caddy_config_set` at the config root (no other delete or set is snapshotted). Whether a path IS the root is `api.isRootConfigPath`, which shares `normalizePath` with the request functions and also absorbs the lone trailing `...` segment Caddy strips before it picks a method. In-memory by default; `CADDY_MCP_SNAPSHOT_DIR` persists them to disk and rehydrates on first access, so rollback survives a restart.
