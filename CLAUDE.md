...
- `src/snapshots.ts` — Ring of the last 10 config snapshots (module-level, per-process). Backs `caddy_revert`; auto-captured before `caddy_load`, and before a `caddy_config_delete` at the config root (no other delete is snapshotted). In-memory by default; `CADDY_MCP_SNAPSHOT_DIR` persists them to disk and rehydrates on first access, so rollback survives a restart.
