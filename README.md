...
- **Safe-by-default mutations** — `caddy_config_set` defaults to idempotent `overwrite` (PATCH), not `append` (POST). Calling twice doesn't duplicate your route. A write to the config root — which replaces the *entire* config, admin endpoint included — is refused without `confirm=true` and snapshotted first, like `caddy_load`.
...
npm test           # Vitest (750 unit tests, +39 POSIX-only unix-socket and launcher tests; +35 live-Caddy integration tests gated by CADDY_MCP_INTEGRATION=1)
